  ...
  const fetchMetadataAndListing = useCallback(async () => {
    try {
      ...
      if (listingJson.success) {
        const item = listingJson.data;
        setName(item.name || '');
        setDescription(item.description || '');
        setCategorySlug(item.category?.slug || '');
        setSubcategory(item.subcategory || '');
        setCitySlug(item.city?.slug || '');
        ...
        setPrice({
          value: item.price?.value !== null ? String(item.price?.value) : '',
          maxValue: item.price?.maxValue !== null ? String(item.price?.maxValue) : '',
          unit: item.price?.unit || 'per_month',
        ...
        setTiming({
          open: item.timing?.open || '09:00',
          close: item.timing?.close || '22:00',
          days: item.timing?.days || 'Mon–Sun',
          is24Hours: item.timing?.is24Hours || false
        });
        setTags(item.tags || []);
        setImages(item.images || []);
      } else {
        setError(listingJson.error || 'Failed to load listing details');
      }
    } catch (err) {
      setError('An error occurred during details compilation');
    } finally {
    ...
    <div className="bg-surface min-h-screen flex flex-col bg-pattern">
      ...
      <main className="flex-grow max-w-3xl w-full mx-auto px-6 py-12">
        <div className="bg-white border border-outline rounded-card p-8 shadow-sm flex flex-col gap-8">
          <div>
            <h1 className="font-plus-jakarta text-2xl font-extrabold text-primary tracking-tight">Edit Verified Listing</h1>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Update parameters for your property, PG, or service</p>
          </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="border-2 border-dashed border-outline/70 rounded-2xl p-6 text-center flex flex-col items-center gap-3">
                ...
                <input 
                  ...
                  className="hidden" id="image-upload-input-edit"
                />
                <label 
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="flex items-center gap-2 pt-6">
                  <input 
                    ...
                    className="accent-primary" id="timing-24h-edit"
                  />
                  <label htmlFor="timing-24h-edit" className="cursor-pointer">Open 24 Hours</label>
                </div>
              </div>
            </div>
